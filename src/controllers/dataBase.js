...
async function upDateDataBaseTeam(req, res) {
	try {
		...
		const queryInsertTeam = `INSERT INTO team (name, image, "backgroundImage_1", "backgroundImage_2", official_page, stadium_id, established, division_id) VALUES ( $1 , $2, $3, $4, $5, $6, $7, $8 ) RETURNING id`;
		let arrayDivisionesClears = [];
		teamsData.forEach((team) => {
			let arrayDivisions = team.division.split(" ");
			let newDivision = arrayDivisions[1] + " " + arrayDivisions[2];
			if (!arrayDivisionesClears.includes(newDivision)) {
				arrayDivisionesClears.push(newDivision);
			}
		});
		const resultDivisions = await getId("division", arrayDivisionesClears);
		teamsData.forEach((team) => {
			const indexDivision = arrayDivisionesClears.findIndex((division) =>
				team.division.includes(division)
			);
			pool
...
async function clearDatabasePlayers() {
	...
	await pool.query("DELETE FROM hometown");
